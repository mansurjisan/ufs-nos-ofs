#!/bin/bash
# =============================================================================
# modify_hrrr_nco.sh - Add ESMF/CF attributes to HRRR NetCDF using NCO
# =============================================================================
#
# Alternative to Python script that uses NCO tools (more reliable on WCOSS2)
# HRRR has 2D lat/lon arrays (Lambert Conformal grid)
#
# Usage:
#   ./modify_hrrr_nco.sh hrrr_raw.nc hrrr_for_esmf.nc
#
# =============================================================================
...
echo "============================================"
echo "Modifying HRRR NetCDF for ESMF (NCO method)"
echo "============================================"
...
ncatted -h -a title,global,o,c,"HRRR 3km data prepared for ESMF mesh generation" "$OUTPUT_FILE"
ncatted -h -a source,global,o,c,"NOAA HRRR (High-Resolution Rapid Refresh)" "$OUTPUT_FILE"
ncatted -h -a grid_type,global,o,c,"Lambert Conformal Conic" "$OUTPUT_FILE"

# HRRR has 2D latitude/longitude arrays
...
DATA_VARS=$(ncdump -h "$OUTPUT_FILE" | grep -E "^\s+(float|double|int)" | grep -v "longitude\|latitude\|time\|x\|y" | awk '{print $2}' | cut -d'(' -f1)
