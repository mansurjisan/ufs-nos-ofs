...
import dates
from mesh_base import Gr3
from jsonencoder import NpEncoder
from nwm_base import SourceSink, Sources, Sinks
...
#logging.basicConfig(filename='kkk.log', level=logging.INFO)  ## machuan added
...
#            print("== mmgp == inside AWSDataInventory and jump to AWSForecastInventory==")
...
class AWSForecastInventory(AWSDataInventory):
    ...
    def __init__(
        ...
        if not os.path.exists(yesterday.strftime("%Y%m%d")):
            logger.info(f"Downloading NWM data mmgp for {yesterday}")
            _ = self.download(yesterday, days=1)
        
        filemaps = self.download(self.start_date, days=rnday.days)  ##  machuan - it seems if there is no downloaded data locally, then will do both days, otherwise not previous day's
...
class NationalWaterModel(SourceSink):
    ...
    print("=== mmgp NationalWaterModel ==")  ## machuan
    ...
    def _fetch_data(
        ...
        print("== mmgp then inside  _fetch_data ==")
        ...
        # XC MOD: DO NOT overwrite with AWSDataInventory!
        # assume self._inventory was set by caller (LocalNWMInventory)
        if not hasattr(self, '_inventory') or self._inventory is None:
            raise RuntimeError("No inventory provided: set nwm._inventory before calling write()")
        ...
        for file in self.inventory.files.values():
            start = datetime.now()
            nc = Dataset(file)
#            print("== mmgp == start ===",start)
#            print("== mmgp === nc file==",file)
...
#        print("==== mmgp i am here inside write ====")
