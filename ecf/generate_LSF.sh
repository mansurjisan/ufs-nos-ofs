...
mkdir -p /lfs/h1/nos/$platform/$LOGNAME/rpt/$nosofs_ver
mkdir -p /lfs/h1/nos/$platform/$LOGNAME/execlog/$nosofs_ver
...
for model in cbofs dbofs tbofs ciofs gomofs leofs lmhofs loofs lsofs ngofs2 sfbofs creofs sscofs
do
  CYC1=00;CYC2=06; CYC3=12; CYC4=18
  if [ $model == "sfbofs" -o $model == "creofs" -o $model == "ngofs2" -o $model == "sscofs" -o -z "${model##wcofs*}" ]; then
...
## WRITE jnos_${model}_aws_${cyc}.pbs
if [ $ftvar = aws ]
then
  ftque=$queue
else
...
do
  ...
  for cyc in $CYC1 $CYC2 $CYC3 $CYC4 ; do
    sed -e "s/MODEL/$model/g" \
        -e "s/QUEUE/dev_transfer/g" \
        -e "s/CYC/$cyc/g" \
        -e "s/PLATFORM/$platform/g" \
        ...
        -e "s/VERSION_FILE/$version_file/g" \
        -e "s/JOB_SCRIPT_FILETRANSFER/$job_script_ft/g" \
        -e "s/PTILE/$ptile/g" nos_${ftvar}.ecf.para > ./new/jnos_${model}_${ftvar}_${cyc}.para
  done
...
do
  ...
  for cyc in $CYC1 $CYC2 $CYC3 $CYC4
  do
    echo "#!/bin/bash" > ./new/subjobs_${model}_${cyc}.sh
    ...
    echo "if [ ! -d /lfs/h1/nos/$platform/$LOGNAME/execlog/$nosofs_ver ]; then " >> ./new/subjobs_${model}_${cyc}.sh
    echo "   mkdir -p /lfs/h1/nos/$platform/$LOGNAME/execlog/$nosofs_ver " >> ./new/subjobs_${model}_${cyc}.sh
    echo "fi " >> ./new/subjobs_${model}_${cyc}.sh

    export RPTDIR=/lfs/h1/nos/$platform/$LOGNAME/rpt/$nosofs_ver
    echo "if [ ! -d $RPTDIR ]; then " >> ./new/subjobs_${model}_${cyc}.sh
    echo "   mkdir -p $RPTDIR " >> ./new/subjobs_${model}_${cyc}.sh
    echo "fi " >> ./new/subjobs_${model}_${cyc}.sh 
...
## create subjobs script for operational file transfer
  for cyc in $CYC1 $CYC2 $CYC3 $CYC4
  do
    echo "#!/bin/bash" > ./new/subjobs_${model}_aws_${cyc}.sh.prod
    echo -e ". $VER" >> ./new/subjobs_${model}_aws_${cyc}.sh.prod
    echo "module purge " >> ./new/subjobs_${model}_aws_${cyc}.sh.prod
    echo "module load envvar/\${envvars_ver:?}" >>  ./new/subjobs_${model}_aws_${cyc}.sh.prod
    echo "module load PrgEnv-intel/\${PrgEnv_intel_ver}" >>  ./new/subjobs_${model}_aws_${cyc}.sh.prod
    echo "module load craype/\${craype_ver}" >>  ./new/subjobs_${model}_aws_${cyc}.sh.prod
    echo "module load intel/\${intel_ver}" >>  ./new/subjobs_${model}_aws_${cyc}.sh.prod 
    export RPTDIR=/lfs/h1/nos/$platform/$LOGNAME/rpt/$nosofs_ver
    echo "if [ ! -d /lfs/h1/nos/$platform/$LOGNAME/execlog ]; then " >> ./new/subjobs_${model}_aws_${cyc}.sh.prod
    echo "   mkdir -p /lfs/h1/nos/$platform/$LOGNAME/execlog " >> ./new/subjobs_${model}_aws_${cyc}.sh.prod
    echo "fi " >> ./new/subjobs_${model}_aws_${cyc}.sh.prod
	      
    export RPTDIR=/lfs/h1/nos/$platform/$LOGNAME/rpt/$nosofs_ver
    echo "if [ ! -d $RPTDIR ]; then " >> ./new/subjobs_${model}_aws_${cyc}.sh.prod
    echo "   mkdir -p $RPTDIR " >> ./new/subjobs_${model}_aws_${cyc}.sh.prod
    echo "fi " >> ./new/subjobs_${model}_aws_${cyc}.sh.prod

    echo "rm -f $RPTDIR/${model}_aws_${cyc}_prod.out"  >> ./new/subjobs_${model}_aws_${cyc}.sh.prod
    echo "rm -f $RPTDIR/${model}_aws_${cyc}_prod.err"  >> ./new/subjobs_${model}_aws_${cyc}.sh.prod
    echo "export LSFDIR=$HOMEnos/pbs " >> ./new/subjobs_${model}_aws_${cyc}.sh.prod
    echo "qsub \$LSFDIR/jnos_${model}_${ftvar}_${cyc}.prod" >> ./new/subjobs_${model}_aws_${cyc}.sh.prod
  done 
  for cyc in $CYC1 $CYC2 $CYC3 $CYC4
  do
    echo "#!/bin/bash" > ./new/subjobs_${model}_aws_${cyc}.sh.para
    echo -e ". $VER" >> ./new/subjobs_${model}_aws_${cyc}.sh.para
    echo "module purge " >> ./new/subjobs_${model}_aws_${cyc}.sh.para
    echo "module load envvar/\${envvars_ver:?}" >>  ./new/subjobs_${model}_aws_${cyc}.sh.para
    echo "module load PrgEnv-intel/\${PrgEnv_intel_ver}" >>  ./new/subjobs_${model}_aws_${cyc}.sh.para
    echo "module load craype/\${craype_ver}" >>  ./new/subjobs_${model}_aws_${cyc}.sh.para
    echo "module load intel/\${intel_ver}" >>  ./new/subjobs_${model}_aws_${cyc}.sh.para
    export RPTDIR=/lfs/h1/nos/$platform/$LOGNAME/rpt/$nosofs_ver
    echo "rm -f $RPTDIR/${model}_aws_${cyc}_para.out"  >> ./new/subjobs_${model}_aws_${cyc}.sh.para
    echo "rm -f $RPTDIR/${model}_aws_${cyc}_para.err"  >> ./new/subjobs_${model}_aws_${cyc}.sh.para
    echo "export LSFDIR=$HOMEnos/pbs " >> ./new/subjobs_${model}_aws_${cyc}.sh.para
    echo "qsub \$LSFDIR/jnos_${model}_${ftvar}_${cyc}.para" >> ./new/subjobs_${model}_aws_${cyc}.sh.para
  done
...
## create subjobs script for NCO parallel run  file transfer
  for cyc in $CYC1
  do
    echo "#!/bin/bash" > ./new/subjobs_${model}_aws_${cyc}.sh.para
    echo -e ". $VER" >> ./new/subjobs_${model}_aws_${cyc}.sh.para
...
do
   sed -e "s/MODEL/$model/g" \
    ...
    echo "#!/bin/bash" > ./new/subjobs_${model}_aws_${cyc}.sh
...
# for ODAAS to transfer ETSS and NAM products
model=odaas
cyc='00'
job_script_ft='\/lfs\/h1\/nos\/nosofs\/noscrub\/\$LOGNAME\/packages\/nosofs\.\${nosofs_ver}\/jobs\/JODAAS_AWS'
...
sed -e "s/MODEL/$model/g" \
    ...
    -e "s/VERSION_FILE/$version_file/g" \
    -e "s/JOB_SCRIPT_FILETRANSFER/$job_script_ft/g" \
    -e "s/PTILE/$ptile/g" nos_${ftvar}.ecf.prod > ./new/jnos_${model}_${ftvar}.prod

echo "#!/bin/bash" > ./new/subjobs_${model}_aws.sh
echo -e ". $VER" >> ./new/subjobs_${model}_aws.sh
echo "module purge " >> ./new/subjobs_${model}_aws.sh
echo "module load envvar/\${envvars_ver:?}" >> ./new/subjobs_${model}_aws.sh
echo "module load PrgEnv-intel/\${PrgEnv_intel_ver}" >> ./new/subjobs_${model}_aws.sh
echo "module load craype/\${craype_ver}" >> ./new/subjobs_${model}_aws.sh
echo "module load intel/\${intel_ver}" >> ./new/subjobs_${model}_aws.sh
echo "export LSFDIR=$HOMEnos/pbs " >> ./new/subjobs_${model}_aws.sh
RPTDIR=/lfs/h1/nos/$platform/$LOGNAME/rpt/$nosofs_ver
echo "rm -f $RPTDIR/${model}_aws_${cyc}_prod.out"  >> ./new/subjobs_${model}_aws.sh
echo "rm -f $RPTDIR/${model}_aws_${cyc}_prod.err"  >> ./new/subjobs_${model}_aws.sh
echo "qsub \$LSFDIR/jnos_${model}_${ftvar}.prod" >> ./new/subjobs_${model}_aws.sh
